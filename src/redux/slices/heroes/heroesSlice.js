...
export const heroesSlice = createSlice({
    ...
    extraReducers: {
        ...
        [fetchHeroes.fulfilled]: (_, action) => ({heroes: action.payload, loading: false}),
        [fetchHeroes.rejected]: (state, _) => ({...state, loading: false}),
        [addHero.fulfilled]: (state, action) => ({...state, heroes: [...state.heroes, action.payload]}),
        [removeHero.fulfilled]: (state, action) => ({...state, heroes: state.heroes.filter(h => h.id !== action.payload.id)}),
  }
})
